...
namespace ift585_tp1
{
    class Frame
    {
        ...
        protected int ttl { get; private set; }
        ...
        public Frame(int id, int type, byte[] data, int ttl = 22)
        {
            this.id = id;
            this.ttl = ttl;
            this.type = type;
        ...
        public void fromBytes(byte[] bytes)
        {
            ...
            byte[] ttl = new byte[4];
            Array.Copy(bytes, 4, ttl, 0, sizeof(int));
            this.ttl = BitConverter.ToInt32(ttl, 0);
        ...
        public byte[] toBytes()
        {
            if (_bytes == null)
            {
                ...
                BitConverter.GetBytes(ttl).CopyTo(frame, 1 * sizeof(int));
        ...
        {
            string s = string.Format("Frame : id={0} ttl={1} type={2} dataLength={3}", id, ttl, type, dataLength);
            string dataString = string.Empty;
            for (int i = 0; i < NB_BYTES; i++)
            {
                dataString += data[i] + " ";
            }
            s += string.Format(" data={0}checksum={1}", dataString, checksum);
            return s;
        }
